...
call make_ohscrcpy_executer.bat
if %errorlevel% neq 0 (
    cd /d "%SCRIPT_DIR%"
    goto :cleanup_on_error
)
...
set OUTPUT_DIR=%SCRIPT_DIR%\Executer\output\Windows\%ARCH%
...
echo.
echo 使用方法：
echo   解压 OHScrcpy_Exec_Windows_%ARCH%_%VERSION%.zip
