...
echo "[信息] 验证打包结果..."
if [ ! -f "dist/OHScrcpy/${EXECUTABLE_NAME}" ]; then
    echo "---------------------------"
...
if [ ! -f "dist/OHScrcpy/_internal/ohscrcpy_server" ]; then
    echo "---------------------------"
...
if [ ! -f "dist/OHScrcpy/_internal/ohscrcpy_server.cfg" ]; then
    echo "---------------------------"
...
chmod +x "dist/OHScrcpy/${EXECUTABLE_NAME}"
...
echo "************************************************"
cp fetch_server_logs.sh dist/OHScrcpy/ 2>/dev/null
cp delete_server_logs.sh dist/OHScrcpy/ 2>/dev/null
cp fetch_and_delete_server_logs.sh dist/OHScrcpy/ 2>/dev/null

echo "[完成] 日志管理脚本已复制到 dist/OHScrcpy/"
ls -la dist/OHScrcpy/*.{sh,bat} 2>/dev/null || echo "[警告] 未找到日志脚本文件"
