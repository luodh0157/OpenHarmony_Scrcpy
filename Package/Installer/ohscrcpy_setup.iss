...
; 安装目录设置
DefaultDirName={autopf}\OpenHarmony_Scrcpy
...
// 返回与批处理逻辑一致的架构字符串
function GetArchTypeStr: String;
var
  ArchW6432, Arch: String;
begin
  // 模拟 PROCESSOR_ARCHITEW6432 检测
  ArchW6432 := GetEnv('PROCESSOR_ARCHITEW6432');
  
  if Length(ArchW6432) > 0 then
    Arch := 'AMD64'          // 批处理中对应 set "ARCH=AMD64"
  else
  ...
  if Arch = 'AMD64' then
  begin
    Result := 'x64';          // 64位 x64 系统
  end
  else if Arch = 'X86' then
  begin
    Result := 'x86';          // 32位 x86 系统
  end
  else if Arch = 'IA64' then
  begin
    Result := 'i64';          // Intel Itanium（极少见）
  end
  else if Arch = 'ARM64' then
  begin
    Result := 'arm64';        // 64位 ARM 系统
  end
  else if Arch = 'ARM' then
  begin
    Result := 'arm';          // 32位 ARM 系统
  end
  else
  begin
    Result := '未知架构：' + Arch;
