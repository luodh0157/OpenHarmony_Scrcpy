...
# ================================================================================
# OpenHarmony OHScrcpy 服务端日志管理脚本 (Linux/Mac - 拉取并删除)
# ================================================================================
#
# 功能：串行执行拉取+删除服务端日志文件
#
# 参数：PID - 服务端进程PID（可选）
#       - 指定PID：只处理该PID对应的日志文件（精确操作）
#       - 不指定PID：处理所有服务端日志文件（批量操作，删除前需要确认）
#
# PID获取方式：
...
#
# 使用示例：
#   ./fetch_and_delete_server_logs.sh 12345  # 拉取并删除PID=12345的日志文件
#   ./fetch_and_delete_server_logs.sh        # 拉取所有日志，然后删除（需确认）
#
# 注意事项：
#   - 自动执行：先拉取到本地，然后删除设备上的日志
#   - 指定PID：拉取后直接删除，无需确认
#   - 不指定PID：拉取所有日志后，删除前需要确认（安全机制）
#   - 多设备连接时会提示选择设备
#
# ================================================================================
...
echo -e "\033[32m     OpenHarmony OHScrcpy 服务端日志拉取并删除脚本（Linux/Mac平台）    \033[0m"
echo -e "\033[32m===============================================================\033[0m"
echo ""
...
echo -e "\033[32m========================================\033[0m"
echo -e "\033[32m第一步：拉取日志到本地\033[0m"
echo -e "\033[32m========================================\033[0m"
...
echo ""
echo -e "\033[32m****************************\033[0m"
echo -e "\033[32m[完成] 已拉取 $FETCH_COUNT 个日志文件到: $DEST_DIR/\033[0m"
echo -e "\033[32m****************************\033[0m"
echo ""
...
echo -e "\033[32m========================================\033[0m"
echo -e "\033[32m第二步：删除设备上的日志\033[0m"
echo -e "\033[32m========================================\033[0m"
...
if [ -n "$PID" ]; then
    ...
    echo -e "\033[32m****************************\033[0m"
    echo -e "\033[32m[完成] 已删除设备上的日志文件\033[0m"
    echo -e "\033[32m****************************\033[0m"
else
    # 不指定PID：批量删除，需要确认
    echo -e "\033[33m-----------------------------------------\033[0m"
    ...
    if [ "$CONFIRM" = "y" ] || [ "$CONFIRM" = "Y" ]; then
        ...
        echo -e "\033[32m****************************\033[0m"
        echo -e "\033[32m[完成] 已删除设备上的所有日志文件\033[0m"
        echo -e "\033[32m****************************\033[0m"
    else
...
echo ""
echo -e "\033[32m****************************\033[0m"
echo -e "\033[32m[完成] 全流程完成\033[0m"
echo -e "\033[32m****************************\033[0m"
echo ""
read -p "按任意键继续..."